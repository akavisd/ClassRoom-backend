		...
		this.createIndex = function(name, successCallback, errorCallback) {
			...
			store.db.transaction(function(tx) {
				sql_table_exists_or_empty(tx, name, function(exists) {  
					if (!exists) {
						tx.executeSql("CREATE TABLE IF NOT EXISTS "+ name +" (id NCHAR(48), value varchar(50), score int(11), INDEX(id), UNIQUE KEY (id, value, score))", [], 
//						tx.executeSql("CREATE TABLE IF NOT EXISTS "+ name +" (id NCHAR(48), value varchar(50), score int(11))", [], 
									function() {
										store.db.transaction(function(tx) {		
										
												
												//tx.executeSql("ALTER TABLE " + name + " ADD INDEX (id)", [], function() {
//												tx.executeSql("INSERT OR REPLACE INTO " + meta.tablename + " (id, initialized) VALUES (?,?)", [name, false], 
												tx.executeSql("REPLACE INTO " + meta.tablename + " (id, initialized) VALUES (?,?)", [name, false], 
		...
		this.setInitialized = function(tablename, value, callback) {
			store.db.transaction(function(tx) {
				tx.executeSql("REPLACE INTO " + meta.tablename + " (id, initialized) VALUES (?,?)", [tablename, value?"true":"false"],
		...
		store.db.transaction(function(tx) {
			tx.executeSql("CREATE TABLE IF NOT EXISTS "+ meta.tablename +" (id VARCHAR(52) NOT NULL PRIMARY KEY, initialized tinyint(1), version VARCHAR(10), ctime DATE)", [], 
				function() {
					callback(store);
	 ...
	 * @constructor
	 */
	fullproof.store.MySQLStore = function(){
		if (!(this instanceof fullproof.store.MySQLStore)) {
			return new fullproof.store.MySQLStore();
		}
	...
	fullproof.store.MySQLStore.getCapabilities = function () {
        try {
            return new fullproof.Capabilities().setStoreObjects(false).setVolatile(false).setAvailable(mySQL.openDatabase).setUseScores([true, false]);
        } catch (e) {
            return new fullproof.Capabilities().setAvailable(false);
        }
    };
	fullproof.store.MySQLStore.storeName = "MySQLStore";

	
	fullproof.store.MySQLStore.prototype.setOptions = function(params) {
	...
	function openStore(store, parameters, callback) {
        ...
        try {
            store.db = mySQL.openDatabase(store.dbName, '1.0', 'javascript search engine', store.dbSize);
	...
	fullproof.store.MySQLStore.prototype.open = function(caps, reqIndexArray, callback, errorCallback) {
        var self = this;
		var resultArray = [];
	...
	fullproof.store.MySQLStore.prototype.close = function(callback) {
		this.internal_init();
		callback();
	};

	fullproof.store.MySQLStore.prototype.getIndex = function(name) {
		return this.indexes[name];
	};

	MySQLStoreIndex.prototype.clear = function(callback) {
		var self = this;
	...
	MySQLStoreIndex.prototype.startBulkInsert = function(callback) {
		var self = this;
		this.db.transaction(function(tx) {
			tx.executeSql("SET AUTOCOMMIT = 0;", [], function(){
				tx.executeSql("SET unique_checks=0;", [], function(){
					self.beginTran(callback);
				});
			});
		});
	}
	...
	MySQLStoreIndex.prototype.finishBulkInsert = function( callback) {
		var self = this;
		this.db.transaction(function(tx) {
			tx.executeSql("COMMIT", [], function(){
				tx.executeSql("SET unique_checks = 1", [], function(){
					tx.executeSql("SET AUTOCOMMIT = 1", [], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
				});
			});
		});
	}
	...
	MySQLStoreIndex.prototype.beginTran = function( callback) {
		var self = this;
		this.db.transaction(function(tx) {
			tx.executeSql("SET AUTOCOMMIT = 0;", [], function(){
				tx.executeSql("START TRANSACTION", [], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
			});
		});
	}
	...
	MySQLStoreIndex.prototype.commitTran = function( callback) {
		var self = this;
		this.db.transaction(function(tx) {
			tx.executeSql("COMMIT;", [], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
		});
	}
	
	
	MySQLStoreIndex.prototype.inject = function(word, value, callback) {
		var self = this;
		this.db.transaction(function(tx) {
			
			if (value instanceof fullproof.ScoredElement) {
				tx.executeSql("REPLACE INTO " + self.tableName + " (id,value, score) VALUES (?,?,?)", [word, value.value, value.score], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
			} else {
				tx.executeSql("REPLACE INTO " + self.tableName + " (id,value) VALUES (?,?)", [word, value], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
			}
/*
			if (value instanceof fullproof.ScoredElement) {
				tx.executeSql("INSERT INTO " + self.tableName + " (id,value, score) VALUES (?,?,?)", [word, value.value, value.score], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
			} else {
				tx.executeSql("INSERT INTO " + self.tableName + " (id,value) VALUES (?,?)", [word, value], fullproof.make_callback(callback, true), fullproof.make_callback(callback, false));
			}
*/
	...
	MySQLStoreIndex.prototype.injectBulk = function(wordArray, valuesArray, callback, progress) {
		var self = this;
...
//			}
	...
	/**
	 * MySQLStore does not support object storage, only primary values, so we rely
	 * on the sql engine sorting functions. ORDER BY should provide fine results as long as
	 * the datatype of values is consistant.
	 */
	MySQLStoreIndex.prototype.lookup = function(word, callback) {
		var self = this;
		this.db.transaction(function(tx) {
